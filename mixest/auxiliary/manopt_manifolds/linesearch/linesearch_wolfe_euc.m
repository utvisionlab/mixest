...
%      For more information see linesearch_wolfe
% 
% This linesearch algorithm can be used only when manifold factories 
% implemented such that representaion of tangent vectors are so that
%       Df(X).H = \nabla f(X)^T H, 
% where H is the representation of a direction in the tangent space
% and \nabla f(X) is the Euclidean gradient
% Most manifolds like Stiefel, Grassman, SPD have this property
% 
%
%
...
% Inpired by the linesearch code of minFunc toolbox
    ...
    if first_run
        ...
        [new_f new_egrad storedb] = getEuclideanCostGrad(problem, newx, storedb);
        if ~is_illegal(new_f)
            new_df = calcDf(M, x, newx, d, new_egrad);
        end
    end
        ...
        if is_illegal(new_f)
            ...
            while true
                ...
                if new_f <= f0 + suff_decr*new_alpha*df0
                    [new_egrad storedb] = getEuclideanGradient(problem, newx, storedb);
                    if ~is_illegal(new_egrad)
                        break;
                    end
                end
    ...
    while ~done && ls_iter < max_ls_steps
        ...
        if first_run
            ...
            %new_df = M.inner(newx, new_d, new_egrad);
        end
    ...
    lsmem.cost = new_f;
    new_grad = problem.M.egrad2rgrad(newx, new_egrad); 
    lsmem.grad = new_grad;
    lsmem.f0 = f0;
    lsmem.df0 = df0;
...
function  new_df = calcDf(M, x, newx, d, new_egrad)
    new_d = M.transp(x, newx, d);
    new_df = obj2vec(new_egrad).'*obj2vec(new_d);
    %new_df = M.inner(newx, new_d, new_grad);
end

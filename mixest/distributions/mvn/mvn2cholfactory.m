...
%
% * *|chols|* (|(datadim+1)-by-(datadim+1)| matrix) : Cholesky factor
% of a matrix containing compound mean and covariance matrix.
% 
% *See also* <mvnfactory.html mvnfactory>
%
% *Point*
% 
% This function is here just for testing purposes (use mvn2icholfactory)
%
% *Example*
%
...
%   theta = struct('chols', [sqrt(2),0,0;0,sqrt(2),0;0,0,1]);
%   % Plot the PDF:
%   x = -5:0.2:5;
%   y = -5:0.2:5;
%   [X, Y] = meshgrid(x, y);
...
function D = mvn2cholfactory(datadim)
...
% See <doc_distribution_common.html#2 distribution structure common members>.
    ...
    cholsM = triufactory(datadim+1,datadim+1);
    D.M = productmanifold(struct('chols', cholsM));
...
%
    ...
    function theta = main2new(theta)
        if isfield(theta,'mu')
            ...
            theta.chols = chol(sigmat); % C=R' R ( R upper trangular)
...
%%
    ...
    % calculate intermediate parameters for ll #1 (parameters depending only on theta)
        if ~isfield(store, 'Rinv') || ~isfield(store, 'logdetR')
            ...
            store.Rinv = theta.chols \ eye(datadim+1); % Faster version of Rinv = inv_triu(R);
            store.logdetR = sum(log(abs(diag(theta.chols))));
        end
...
% See <doc_distribution_common.html#7 distribution structure common members>.
    ...
    function [dll, store] = llgrad(theta, data, store)
        ...
        dll.chols = -diag(sumW ./diag(chols)) + Rinv.' * DDT * Sinv;
        dll.chols = triu(dll.chols);
    end
...
% See <doc_distribution_common.html#18 distribution structure common members>.
    ...
    function theta = sumparam(theta1, theta2)
        theta.chols = theta1.chols + theta2.chols;
    end
...
% See <doc_distribution_common.html#19 distribution structure common members>.
    ...
    function theta = scaleparam(scalar, theta)
        theta.chols = scalar * theta.chols;
    end
...
% See <doc_distribution_common.html#22 distribution structure common members>.
    ...
    function h = entropy(theta)
        logdetR = sum(log( abs(diag( theta.chols )) )); 
...
% See <doc_distribution_common.html#23 distribution structure common members>.
    ...
    function kl = kl(theta1, theta2)
        ...
        U = theta1.chols; % Y.sigmat = U'*U (U is upper-traingular)
        V = eye(datadim+1) \ theta2.chols; % V = inv_triu(V);
...
% See <doc_distribution_common.html#26 distribution structure common members>.
    ...
    function str = display(theta)
        str = '';
        str = [str, sprintf('chol (%d-by-%d): %s\n', size(theta.chols,1), size(theta.chols,2), mat2str(theta.chols, 4))];
